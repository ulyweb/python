...
#!/usr/bin/env python3
"""
seed_vault_gui.py - GUI for per-word seed phrase vault (12/24 words)

Features:
- Encrypt tab:
  - Paste/type 12 or 24 words; preview them
  - Choose how many to secure (per-word codes) and selection method
  - Choose how remaining words are stored: view-tier (view PIN or master-only) or plaintext (NOT recommended)
  - Custom vault filename
  - Final masked preview before saving
  - Per-word code collection dialog for secured words
- Show tab: display all words (view/plaintext decrypted; secured shown as [LOCKED] or optionally decrypted one-by-one)
- Decrypt tab: decrypt a single word by index
- List tab: list indices and tiers

Crypto:
- AES-GCM for encryption
- scrypt for key derivation
- Keys bind to index to ensure unique keys per word

DISCLAIMER:
- Storing recovery phrases electronically is risky. Use long master passphrases and keep vaults offline.
- Test with dummy phrases first. Keep per-word codes separate from the vault.
"""
...
try:
    ...
    raise SystemExit("Missing 'cryptography' package. Install via: python -m pip install cryptography")
...
class CodesDialog(tk.Toplevel):
    """Collect per-word codes for secured indices."""
...
class SeedVaultApp(tk.Tk):
    ...
    def build_encrypt_tab(self):
        ...
        # Master PIN
        ttk.Label(frm, text="Master PIN/passphrase (recommended):").grid(row=0, column=0, sticky="w", padx=10, pady=5)
        ...
        ttk.Label(frm, text="Remaining words storage:").grid(row=1, column=0, sticky="w", padx=10, pady=5)
        ...
        rb1.grid(row=1, column=1, sticky="w", padx=10, pady=5)
        rb2.grid(row=2, column=1, sticky="w", padx=10, pady=2)
        rb3.grid(row=3, column=1, sticky="w", padx=10, pady=2)

        ttk.Label(frm, text="View PIN (if using view PIN):").grid(row=4, column=0, sticky="w", padx=10, pady=5)
        self.view_pin_e.grid(row=4, column=1, sticky="we", padx=10, pady=5)

        # Vault filename
        ttk.Label(frm, text="Vault filename (.json):").grid(row=5, column=0, sticky="w", padx=10, pady=5)
        self.vault_name_e = ttk.Entry(frm)
        self.vault_name_e.insert(0, "seed_vault.json")
        self.vault_name_e.grid(row=5, column=1, sticky="we", padx=10, pady=5)

        # Seed words input
        ttk.Label(frm, text="Seed words (paste or type; must be exactly 12 or 24 words):").grid(row=6, column=0, columnspan=2, sticky="w", padx=10, pady=5)
        self.words_text = tk.Text(frm, height=8)
        self.words_text.grid(row=7, column=0, columnspan=2, sticky="nsew", padx=10, pady=5)
        frm.rowconfigure(7, weight=1)

        parse_btn = ttk.Button(frm, text="Parse & Preview", command=self.on_parse_preview)
        parse_btn.grid(row=8, column=0, padx=10, pady=5, sticky="w")

        # Preview list
        ttk.Label(frm, text="Preview of entered words:").grid(row=9, column=0, columnspan=2, sticky="w", padx=10, pady=5)
        self.preview_list = tk.Listbox(frm, height=10)
        self.preview_list.grid(row=10, column=0, columnspan=2, sticky="nsew", padx=10, pady=5)
        frm.rowconfigure(10, weight=1)

        # Secured selection controls
        ttk.Label(frm, text="How many words to secure with per-word codes:").grid(row=11, column=0, sticky="w", padx=10, pady=5)
        ...
        self.secure_count_spin.grid(row=11, column=1, sticky="w", padx=10, pady=5)

        ttk.Label(frm, text="Selection method:").grid(row=12, column=0, sticky="w", padx=10, pady=5)
        ...
        self.select_method_cb.grid(row=12, column=1, sticky="w", padx=10, pady=5)

        ttk.Label(frm, text="Selection details (indices e.g. 1,3,5 or words on new lines):").grid(row=13, column=0, sticky="w", padx=10, pady=5)
        self.select_details_text = tk.Text(frm, height=4)
        self.select_details_text.grid(row=13, column=1, sticky="we", padx=10, pady=5)

        preview_sel_btn = ttk.Button(frm, text="Preview Secured Masking", command=self.on_preview_masking)
        preview_sel_btn.grid(row=14, column=0, padx=10, pady=5, sticky="w")

        ttk.Label(frm, text="Final masked preview:").grid(row=15, column=0, columnspan=2, sticky="w", padx=10, pady=5)
        self.masked_list = tk.Listbox(frm, height=10)
        self.masked_list.grid(row=16, column=0, columnspan=2, sticky="nsew", padx=10, pady=5)
        frm.rowconfigure(16, weight=1)

        encrypt_btn = ttk.Button(frm, text="Encrypt & Save Vault", command=self.on_encrypt_save)
        encrypt_btn.grid(row=17, column=0, columnspan=2, padx=10, pady=10, sticky="e")
    ...
    def compute_secured_indices(self) -> Optional[List[int]]:
        ...
        # Need selection details for indices/words
        if method == "first":
    ...
    def on_encrypt_save(self):
        ...
        # Confirm final masked preview
        ok = messagebox.askyesno("Confirm", f"Proceed to encrypt and save vault to:\n{vault_name}?")
        if not ok:
            return
    ...
    def build_show_tab(self):
        ...
        ttk.Label(frm, text="Master PIN:").grid(row=1, column=0, sticky="w", padx=10, pady=5)
        ...
        for e in entries:
            ...
            if tier == "plaintext":
                ...
                try:
                    ...
                    self.show_list.insert("end", f"{idx:2d}. [FAILED VIEW DECRYPT]")
            elif tier == "secured":
    ...
    def on_decrypt_secured_batch(self):
        ...
        for e in sorted(secured, key=lambda x: x["index"]):
            ...
            try:
                ...
                self.show_list.insert("end", f"{idx:2d}. [FAILED: {ex}]")
    ...
    def build_decrypt_tab(self):
        ...
        ttk.Label(frm, text="Master PIN:").grid(row=2, column=0, sticky="w", padx=10, pady=5)
    ...
    def on_decrypt_one(self):
        ...
        try:
            ...
            if tier == "secured":
                ...
                key = derive_view_key(master, code, idx, salt)
            else:
            ...
            self.dec_result_lbl.configure(text=f"Failed to decrypt: {e}")
