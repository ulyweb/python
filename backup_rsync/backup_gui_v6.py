...
class BackupApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Backup Tool (rsync GUI) ver6")
    ...
    def build_ui(self):
        ...
        # Source row
        tk.Label(top_frame, text="Source Path (file or folder):").grid(row=0, column=0, sticky="w")
        self.src_entry = tk.Entry(top_frame, width=60)
        self.src_entry.grid(row=0, column=1, sticky="ew", padx=5)

        # Buttons: A (popup), B (file), C (folder)
        btn_frame = tk.Frame(top_frame)
        btn_frame.grid(row=0, column=2, sticky="w")

        tk.Button(btn_frame, text="Browse", command=self.pick_source_popup, width=8).pack(side=tk.LEFT, padx=2)
        tk.Button(btn_frame, text="File", command=self.pick_source_file, width=6).pack(side=tk.LEFT, padx=2)
        tk.Button(btn_frame, text="Folder", command=self.pick_source_folder, width=8).pack(side=tk.LEFT, padx=2)
        ...
        tk.Button(top_frame, text="Browse", command=self.pick_destination, width=8).grid(row=1, column=2, padx=5, pady=(5, 0))

        # Default destination
        default_dest = "rbackup@10.17.76.15:/TM-NAS/media/DATA/Recent-Transfer/2026"
        self.dst_entry.insert(0, default_dest)

        # Notes as selectable Text (read‑only)
        notes_text = (
            "Notes: Verion 6.0\n"
            "• Local destination example: /mnt/storage or Z:\\Backups\n"
            "• Remote SSH example: rbackup@10.17.76.15:/TM-NAS/media/DATA/Recent-Transfer/2026\n"
            "• Remote mode requires SSH key authentication (no password prompts).\n"
            "• The remote folder must exist or rsync will fail.\n"
            "• If SSH asks for a password in a terminal, fix SSH keys first.\n"
            "• You can back up a single file OR an entire folder.\n"
        )
        self.notes = tk.Text(top_frame, height=6, wrap="word")
        self.notes.grid(row=2, column=0, columnspan=3, sticky="nsew", pady=(5, 0))
        self.notes.insert("1.0", notes_text)
        self.notes.config(state="disabled")  # read‑only but selectable
    ...
    def _apply_theme_recursive(self, widget, bg, fg, entry_bg, entry_fg):
        ...
        if cls in ("Frame", "LabelFrame"):
            ...
            if "Start" in text:
                widget.configure(bg="#4CAF50", fg="white", activebackground="#45a049")
            elif "Cancel" in text:
                widget.configure(bg="#f44336", fg="white", activebackground="#e53935")
            elif "Quit" in text:
                widget.configure(bg="#555555", fg="white", activebackground="#444444")
            else:
                widget.configure(bg="#777777", fg="white", activebackground="#666666")
        elif cls == "Entry":
    ...
    # ---------------------------------------------------
    # Source pickers (A, B, C)
    # ---------------------------------------------------
    def pick_source_popup(self):
    ...
    def run_rsync(self, src, dst):
        ...
        # Handle file vs directory: only add trailing slash for directories
